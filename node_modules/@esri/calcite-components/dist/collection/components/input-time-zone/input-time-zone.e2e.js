...
describe("calcite-input-time-zone", () => {
    ...
    describe("mode", () => {
        describe("offset (default)", () => {
            ...
            it("allows users to preselect a time zone offset", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone value="${testTimeZoneItems[1].offset}"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
            ...
            it("ignores invalid values", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone value="9000"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
            ...
            it("omits filtered or non-localized time zones (incoming to browser)", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone value="600"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
            ...
            it("looks up in label and time zone groups (not displayed)", async () => {
                ...
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
        ...
        describe("name", () => {
            describe("selects user's matching time zone name on initialization", () => {
                testTimeZoneItems.forEach(({ name }) => {
                    it(`selects default time zone for "${name}"`, async () => {
                        const page = await newE2EPage();
                        await page.emulateTimezone(name);
                        await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone mode="name"></calcite-input-time-zone>`));
                        await page.waitForChanges();
                        const input = await page.find("calcite-input-time-zone");
            ...
            it("allows users to preselect a time zone by name", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone mode="name" value="${testTimeZoneItems[1].name}"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
            ...
            it("ignores invalid values", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await overrideSupportedTimeZones(html `<calcite-input-time-zone mode="name" value="Does/Not/Exist"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
    ...
    describe("clearable", () => {
        it("does not allow users to deselect a time zone value by default", async () => {
            const page = await newE2EPage();
            await page.emulateTimezone(testTimeZoneItems[0].name);
            await page.setContent(overrideSupportedTimeZones(html `
        ...
        describe("clearing by value", () => {
            ...
            beforeEach(async () => {
                ...
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(overrideSupportedTimeZones(html ` <calcite-input-time-zone value="${testTimeZoneItems[1].offset}" clearable></calcite-input-time-zone>`));
        ...
        it("can be cleared on initialization when clearable is enabled", async () => {
            const page = await newE2EPage();
            await page.emulateTimezone(testTimeZoneItems[0].name);
            await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone value="" clearable></calcite-input-time-zone>`));
            const input = await page.find("calcite-input-time-zone");
            expect(await input.getProperty("value")).toBe("");
        });
        it("selects user time zone value when value is not set and clearable is enabled", async () => {
            const page = await newE2EPage();
            await page.emulateTimezone(testTimeZoneItems[0].name);
            await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone clearable></calcite-input-time-zone>`));
            const input = await page.find("calcite-input-time-zone");
            expect(await input.getProperty("value")).toBe(`${testTimeZoneItems[0].offset}`);
        });
    });
    describe("selection of subsequent items with the same offset", () => {
        ...
        testCases.forEach(({ name, initialTimeZoneItem }) => {
            it(`${name}`, async () => {
                ...
                await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone value="${initialTimeZoneItem.offset}"></calcite-input-time-zone> `));
                const input = await page.find("calcite-input-time-zone");
                await input.click();
                await page.waitForChanges();
    ...
    it("supports setting maxItems to display", async () => {
        const page = await newE2EPage();
        await page.emulateTimezone(testTimeZoneItems[0].name);
        await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone max-items="7"></calcite-input-time-zone>`));
    ...
    describe("offsetStyle", () => {
        ...
        let page;
        async function assertItemLabelMatches(page, offsetMarker) {
        ...
        describe("displays UTC or GMT based on user's locale (default)", () => {
            it("displays GMT for GMT-preferred locale", async () => {
                await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone lang="${gmtTimeZoneLocale}"></calcite-input-time-zone>`));
                await assertItemLabelMatches(page, "GMT");
            });
            it("displays UTC for UTC-preferred locale", async () => {
                await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone lang="${utcTimeZoneLocale}"></calcite-input-time-zone>`));
                await assertItemLabelMatches(page, "UTC");
            });
        });
        it("supports GMT as a style", async () => {
            await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone lang="${utcTimeZoneLocale}" offset-style="gmt"></calcite-input-time-zone>`));
            await assertItemLabelMatches(page, "GMT");
        });
        it("supports UTC as a style", async () => {
            await page.setContent(overrideSupportedTimeZones(html `<calcite-input-time-zone lang="${gmtTimeZoneLocale}" offset-style="utc"></calcite-input-time-zone>`));
            await assertItemLabelMatches(page, "UTC");
        });
    });
});
 ...
 * @param testHtml
 */
function overrideSupportedTimeZones(testHtml) {
