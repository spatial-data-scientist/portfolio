...
import { createTransitionEventDispatcher } from "../tests/spec-helpers/transitionEvents";
import { createAnimationEventDispatcher } from "../tests/spec-helpers/animationEvents";
import { mockGetComputedStyleFor } from "../tests/spec-helpers/computedStyle";
import { guidPattern } from "./guid.spec";
import { ensureId, focusElementInGroup, getElementProp, getModeName, getShadowRootNode, getSlotted, isBefore, isKeyboardTriggeredClick, isPrimaryPointerButton, setRequestedIcon, slotChangeGetAssignedElements, slotChangeGetAssignedNodes, slotChangeGetTextContent, slotChangeHasAssignedElement, slotChangeHasAssignedNode, slotChangeHasContent, slotChangeHasTextContent, toAriaBoolean, whenAnimationDone, whenTransitionDone, } from "./dom";
describe("dom", () => {
    ...
    async function promiseState(promise) {
        ...
        return Promise.race([promise, pendingState]).then((value) => (value === pendingState ? value : { status: "fulfilled", value }), (reason) => ({ status: "rejected", reason }));
    }
